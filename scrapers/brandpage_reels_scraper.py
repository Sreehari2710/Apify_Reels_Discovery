...
# ----------------------------
# Celery Task for Brandpage Reels Scraper
# ----------------------------
@celery.task(bind=True)
def scrape_brandpage_reels_task(self, brand_pages: list, results_limit: int):
...
def brandpage_reels():
    try:
        ...
        task = scrape_brandpage_reels_task.delay(brandpages, results_limit)

        return jsonify({"message": "Scraping task initiated", "task_id": task.id}), 202
