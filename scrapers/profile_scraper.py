...
# ----------------- Google Sheet Helpers (Removed - now in app.py) -----------------
...
# ----------------- Fetch All Profiles (Celery Task) -----------------
@celery.task(bind=True)
def filter_and_scrape_profiles_task(self, csv_file_content: str, filename_base: str, form_data: dict):
    try:
        ...
        if {"hashtag", "username", "user_link", "caption_text"}.issubset(headers):
            ...
            self.update_state(state='FAILURE', meta={'status': 'Unrecognized CSV format'})
            return None # Indicate failure

        usernames = list(set(usernames))
        if not usernames:
            self.update_state(state='FAILURE', meta={'status': 'No valid usernames found in CSV'})
            return None
        ...
        for p in profiles:
            ...
            query_value = query_map.get(username, form_data.get("query", "")) # Use form_data for query
...
# ----------------- Main Filter Route (Triggers Celery Task) -----------------
...
def filter_csv():
    try:
        ...
        csv_content = csv_file.stream.read().decode("utf-8") # Read content for Celery task
        filename_base = (request.form.get("filename") or "filtered_profiles")
        form_data = request.form.to_dict() # Pass form data to task

        # Dispatch the scraping task to Celery
        task = filter_and_scrape_profiles_task.delay(csv_content, filename_base, form_data)

        return jsonify({"message": "Profile filtering task initiated", "task_id": task.id}), 202

    except Exception as e:
        return Response(f"Error initiating profile filtering task: {str(e)}", status=500)
