...
from flask import Blueprint, request, send_file, Response, jsonify # Added jsonify
from utils import normalize_hashtags, parse_csv_column, make_apify_request
from config import APIFY_TOKEN, HASHTAG_ACTOR_ID
from app import celery, append_to_gsheet # Import celery from app

bp_hashtag = Blueprint("hashtag_scraper", __name__)

# ----------------------------
# Celery Task for Hashtag Scraper
# ----------------------------
@celery.task(bind=True)
def scrape_hashtag_task(self, tags: t.List[str], max_items: int):
    results = []
...
# ----------------------------
# Flask Route: /fetch (Triggers Celery Task)
# ----------------------------
...
def fetch_hashtag():
    try:
        ...
        task = scrape_hashtag_task.delay(tags, max_items)

        return jsonify({"message": "Hashtag scraping task initiated", "task_id": task.id}), 202
